...
TaskHandle_t GestureSensorTaskHandle = NULL;
...
void GestureSensorTask(void *param)
{
	...
	/*Send initial state to the screen*/
	vTaskDelay(pdMS_TO_TICKS(100));
	gesture_data.gesture = GESTURE_NONE;
	pca9554_dir_set(PCA9554_DIR_NONE);
	while( xTaskNotifyAndQuery(SmartDisplayTaskHandle,sigGESTURE,eSetValueWithoutOverwrite, &ulPreviousValue ) != pdPASS)
	{
	...
	for(;;)
	{
    	...
    	/* Check if the Gesture is still valid */
    	currentTime = xTaskGetTickCount()* (1000/configTICK_RATE_HZ);
    	if(currentTime >= (gesture_data.timestamp + GESTURE_HOLD_MS))
    	{
    		/* GESTURE - NONE is valid indefinitely */
    		gesture_data.timestamp = 0xFFFFFFFF - 2*GESTURE_HOLD_MS;
    		gesture_data.gesture = GESTURE_NONE;
    		pca9554_dir_set(PCA9554_DIR_NONE);
			while( xTaskNotifyAndQuery(SmartDisplayTaskHandle,sigGESTURE,eSetValueWithoutOverwrite, &ulPreviousValue ) != pdPASS)
			{
    	...
    	if(gesture_data.buff_pos == GESTURE_FRAME_SIZE)
    	{
    		...
    		if((gesture_data.std_left >= STD_THRESHOLD) || (gesture_data.std_right >= STD_THRESHOLD) || (gesture_data.std_up >= STD_THRESHOLD))
    		{
        		...
        		{
        			gesture_data.timestamp = xTaskGetTickCount()*(1000/configTICK_RATE_HZ);
